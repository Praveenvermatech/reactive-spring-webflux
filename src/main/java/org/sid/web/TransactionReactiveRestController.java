...
public class TransactionReactiveRestController {
	...
	/*
	 * @GetMapping(value = "/events/{id}",produces =
	 * MediaType.TEXT_EVENT_STREAM_VALUE) public Flux<Double> events(@PathVariable
	 * String id){ WebClient webClient=WebClient.create("http://localhost:8082");
	 * Flux<Double> eventFlux=webClient.get() .uri("/streamEvents/"+id)
	 * .retrieve().bodyToFlux(Event.class) .map(data->data.getValue()); return
	 * eventFlux;
	 * 
	 * }
	 */
